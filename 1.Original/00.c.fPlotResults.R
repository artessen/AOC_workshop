...
pltSHAP = plot_shap_beeswarm(dfSHAP[[1]][[1]], dfSHAP[[2]][[1]], size=1.0, cex=0.15, sort=T,
...
limits = c(0,1)
dfData = read.csv(paste("./xgb_model_predictions_",sAOC,".csv",sep=""))
...
pltDriver = ggplot() +
  ...
  labs(title=sVarImp, y=NULL, x=NULL, colour=NULL) + # , colour=sVar) +
  theme_bw() + 
  theme(plot.title = element_text(size = 14, face = "bold"), 
        panel.grid.major = element_line(color = gray(.5), linetype = "dashed", linewidth = 0.1), 
        panel.background = element_rect(fill = "white"), 
