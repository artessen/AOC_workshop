# Function to train an XGBoost model for detecting AOCs
fTrainModel = function(Target, Features, TrainingAttempts=3) {
  ...
  # sCEMS = c("spi01","spi03","spi06","spi09","spi12", "smant", "fpanv")
  sERA5mean = Features[[1]]
  sERA5anom = Features[[1]]
  sERA5spec = Features[[2]]
  sCEMS = Features[[3]]
  ...
  for (i in 1:length(Target)) {
    lFeats[[i]] = c(sERA5mean, sERA5anom, sERA5spec, sCEMS)
  }
  ...
  for (i in 1:length(sTgts)) {
    ...
    for (att in 1:atts) {
      ...
      repeat {
        ...
        if (xgbTree_model$best_score > 0.0) { break }  # attributes
      }
      # ==========================================================================
      ...
      shapExp = xgb.plot.shap(dfSHAPsm, model=xgbTree_model, plot=F, top_n=dim(dfSHAPsm)[2], subsample=1) # subsample = NULL
